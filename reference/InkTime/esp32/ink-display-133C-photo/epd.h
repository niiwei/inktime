...
// #define PIN_SPI_PWR  32
// #else
// #define PIN_SPI_SCK  9
// #define PIN_SPI_DIN  10
// #define PIN_SPI_CS   6
// #define PIN_SPI_BUSY 17
// #define PIN_SPI_RST  18
// #define PIN_SPI_DC   8
// #define PIN_SPI_CS_S 7
// #define PIN_SPI_CS_M 6
// #define PIN_SPI_PWR  5
// #endif


#define _32s    //_32s    _32s3
#ifdef _32s
// SPI引脚定义（墨水屏）
#define PIN_SPI_PWR     32
#define PIN_SPI_BUSY   25
...
#define PIN_SPI_DIN  14
//蜂鸣器
#define BEEP  12//PWM高电平起作用，默认拉低
// 按键定义
#define KEY_UP    34
#define KEY_MID   35
#define KEY_DOWN  0
// TF卡（SD卡）SPI引脚定义（如使用SPI模式）
#define PIN_TF_CS      5   // SD卡片选
#define PIN_TF_MOSI    23  // SD卡MOSI
#define PIN_TF_SCK     18  // SD卡SCK
#define PIN_TF_MISO    19  // SD卡MISO
//定义蓝色led引脚
...
#else    //_32s3
// SPI引脚定义（墨水屏）
#define PIN_SPI_PWR     12//32
#define PIN_SPI_BUSY 17//25
#define PIN_SPI_RST  18//26
#define PIN_SPI_DC   8//27
#define PIN_SPI_CS_S    16//33
#define PIN_SPI_CS_M    47//15
#define PIN_SPI_CS   47//15
#define PIN_SPI_SCK  9//13
#define PIN_SPI_DIN  19//14
//蜂鸣器
#define BEEP  20//PWM高电平起作用，默认拉低
// 按键定义
#define KEY_UP    6//34
#define KEY_MID   7//35
#define KEY_DOWN  0
// TF卡（SD卡）SPI引脚定义（如使用SPI模式）
#define PIN_TF_CS      38//5   // SD卡片选
#define PIN_TF_MOSI    39//23  // SD卡MOSI
#define PIN_TF_SCK     40//18  // SD卡SCK
#define PIN_TF_MISO    41//19  // SD卡MISO
//定义蓝色led引脚
#define PIN_BLUE_LED     48  //blue LED
#endif
...
void EPD_initSPI()
{
    ...
    pinMode(PIN_SPI_PWR , OUTPUT);  //外设电源

    //默认值
    digitalWrite(PIN_SPI_CS, HIGH);
    digitalWrite(PIN_SPI_CS_S, LOW);         //ESP32S的话，修改为0，即可关闭蓝色LED
    digitalWrite(PIN_SPI_PWR, HIGH);         //打开外设供电
    digitalWrite(PIN_SPI_SCK, LOW);
    digitalWrite(BEEP, 0);                   //蜂鸣器默认关闭 
    digitalWrite(PIN_BLUE_LED, 0);           //蓝色LED灭
    vspi.begin(PIN_SPI_SCK, -1, PIN_SPI_DIN, -1);
}
...
void EpdSpiTransferCallback(byte data) 
{
    ...
    for (int i = 0; i < 8; i++)
    {
        digitalWrite(PIN_SPI_DIN, ((data & 0x80) == 0 ? GPIO_PIN_RESET : GPIO_PIN_SET));
        data <<= 1;
...
void(*EPD_dispLoad)(const uint8_t*, size_t);     // Pointer on a image data writting function

/* Image data loading function for a-type e-Paper ----------------------------*/ 
void EPD_loadA(const uint8_t* s, size_t bytes)
{
...
void EPD_loadAFilp(const uint8_t* s, size_t bytes)
{
    // Enumerate all of image data bytes
    for (int pos = 0; pos < bytes; ++pos)
    {
...
void EPD_loadB(const uint8_t* s, size_t bytes)
{
    // Enumerate all of image data bytes
    for (int pos = 0; pos + 1 < bytes; pos += 2)
    {
        // Get current word from obtained image data
        int valueA = (int)s[pos] + ((int)s[pos + 1] << 8);
...
void EPD_loadC(const uint8_t* s, size_t bytes)
{
...
void EPD_loadD(const uint8_t* s, size_t bytes)
{
    // Enumerate all of image data bytes
    for (int pos = 0; pos < bytes; ++pos)
    {
        // Get current byte from obtained image data
        uint8_t valueA = s[pos];
...
void EPD_loadE(const uint8_t* s, size_t bytes)
{
    // Enumerate all of image data bytes
    for (int pos = 0; pos < bytes; ++pos)
    {
...
void EPD_loadF(const uint8_t* s, size_t bytes)
{
    // Enumerate all of image data bytes
    for (int pos = 0; pos + 1 < bytes; pos += 2)
    {
        // Get current byte from obtained image data
        uint8_t value = s[pos];
        uint8_t value1 = s[pos+1];
        // Processing of 4 1-bit pixels to 4 4-bit pixels:
        // white(value 1) to bits 0011, red(value 2) to bits 0100
        int A = (value     ) & 3;if (A == 2) A = 4;
        int B = (value >> 4) & 3;if (B == 2) B = 4; // Not sure why it was value>>8
...
void EPD_loadG(const uint8_t* s, size_t bytes)
{
    // Enumerate all of image data bytes
    for (int pos = 0; pos < bytes; ++pos)
    {
        ...
        uint8_t A = (value     ) & 0x07;
        uint8_t B = (value >> 4) & 0x07;
...
void EPD_load_13in3E6(const uint8_t* s, size_t bytes)
{
    digitalWrite(PIN_SPI_DC, HIGH);
    vspi.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE0));
    // Enumerate all of image data bytes - direct bulk transfer
    for (int pos = 0; pos < bytes; ++pos)
    {
        // Get current byte from obtained image data
        uint8_t value = s[pos];
        ...
        uint8_t A = (value     ) & 0x07;
        uint8_t B = (value >> 4) & 0x07;
        // Write the data into e-Paper's memory - direct transfer without toggling CS
        vspi.transfer((A << 4) + B);
    }
    vspi.endTransaction();
}
...
struct EPD_dispInfo
{
    ...
    void(*chBk)(const uint8_t*, size_t);// Black channel loading
    int next;     // Change channel code
    void(*chRd)(const uint8_t*, size_t);// Red channel loading
